...
<template>
  <v-card :loading="loading">
    ...
    <v-card-title>
      ...
      {{ $t('admin-room-update-description')}}
      <v-spacer />
    ...
    <v-slide-y-transition>
      <v-container fluid v-if="showHelp">
        <v-row>
          <v-col class="px-4">
            <p>{{ $t('admin-room-update-description-help') }}</p>
          </v-col>
        </v-row>
      </v-container>
    </v-slide-y-transition>
...
export default {
  ...
  computed: {
    ...
    descriptionLabel () {
      return this.$i18n.t('admin-room-description-label');
    },

    descriptionHint () {
      return this.$i18n.t('admin-room-description-hint');
    },
  },
