<!--
Admin - Room - Update Language Id

A smart component that enables an Admin to update the Language used by a Room.
-->
<template>
  <v-card :loading="loading">
    ...
    <v-card-title>
      ...
      {{ $t('admin-room-update-language-id')}}
      <v-spacer />
    ...
    <v-slide-y-transition>
      <v-container fluid v-if="showHelp">
        <v-row>
          <v-col class="px-4">
            <p>{{ $t('admin-room-update-language-id-help') }}</p>
          </v-col>
        </v-row>
      </v-container>
    </v-slide-y-transition>
