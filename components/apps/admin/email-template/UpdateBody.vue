...
<template>
  <v-card :loading="loading">
    ...
    <v-card-title>
      ...
      {{ $t('admin-email-template-update-body')}}
      <v-spacer />
    ...
    <v-card-subtitle>{{ $t('admin-email-template-update-body-subtitle') }}</v-card-subtitle>
    ...
    <v-slide-y-transition>
      <v-container fluid v-if="showHelp">
        <v-row>
          <v-col class="px-4">
            <p>{{ $t('admin-email-template-update-body-help') }}</p>
          </v-col>
        </v-row>
      </v-container>
    </v-slide-y-transition>
    ...
    <v-container fluid>
      <v-row>
        <v-col class="px-4">
          <pro-text-editor v-model="textLocal" />
      ...
      <v-row>
        <v-col class="px-4">
          <v-btn large
                 ...
                 :loading="submitting"
                 :disabled="submitting">
            {{ $t('admin-email-template-update-body-submit') }}
          </v-btn>
        </v-col>
      </v-row>
    </v-container>
...
export default {
      ...
      handler () {
        if (this.bodyId) {
          if (! this.labelLoading && ! this.labelLoaded) {
            this.loadLabelWithLabelId(this.bodyId);
          }
        }
      }
    },
  ...
  computed: {
    ...
    label () {
      return (this.bodyId) ? this.labelWithLabelId(this.bodyId) : undefined;
    },
    ...
    textLabel () {
      return this.$i18n.t('admin-email-template-text-label');
    },

    textHint () {
      return this.$i18n.t('admin-email-template-text-hint');
    },
  },
